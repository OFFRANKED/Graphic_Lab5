...
void LitColumnsApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData cylinder = geoGen.CreateCylinder(0.5f, 0.0f, 3.0f, 4, 1);
	GeometryGenerator::MeshData diamond = geoGen.CreateDiamond(5, 3);
	GeometryGenerator::MeshData pyramid = geoGen.CreatePyramid(1, 1);
	GeometryGenerator::MeshData wedge = geoGen.CreateWedge(1, 1,1);
	GeometryGenerator::MeshData battlement = geoGen.CreateBattlements(2, 1.96, 4);
	GeometryGenerator::MeshData tetrahedron = geoGen.CreateTetraHedron(2, 2, 2);
	GeometryGenerator::MeshData parrellegram = geoGen.CreateParrellegramPrism(2, 5, 1);
	//
	...
	UINT battlementVertexOffset = wedgeVertexOffset + (UINT)wedge.Vertices.size();
	UINT tetraVertexOffset = battlementVertexOffset + (UINT)battlement.Vertices.size();
	UINT parrellegramVertexOffset = tetraVertexOffset + (UINT)tetrahedron.Vertices.size();
	...
	UINT battlementIndexOffset = wedgeIndexOffset + (UINT)wedge.Indices32.size();
	UINT tetraIndexOffset = battlementIndexOffset + (UINT)battlement.Indices32.size();
	UINT parrellegramIndexOffset = tetraIndexOffset + (UINT)tetrahedron.Indices32.size();
	...
	SubmeshGeometry battlementSubmesh;
	battlementSubmesh.IndexCount = (UINT)battlement.Indices32.size();
	battlementSubmesh.StartIndexLocation = battlementIndexOffset;
	battlementSubmesh.BaseVertexLocation = battlementVertexOffset;

	SubmeshGeometry tetraSubmesh;
	tetraSubmesh.IndexCount = (UINT)tetrahedron.Indices32.size();
	tetraSubmesh.StartIndexLocation = tetraIndexOffset;
	tetraSubmesh.BaseVertexLocation = tetraVertexOffset;

	SubmeshGeometry parrellegramSubmesh;
	parrellegramSubmesh.IndexCount = (UINT)parrellegram.Indices32.size();
	parrellegramSubmesh.StartIndexLocation = parrellegramIndexOffset;
	parrellegramSubmesh.BaseVertexLocation = parrellegramVertexOffset;
	//
	...
	auto totalVertexCount =
		...
		wedge.Vertices.size()+
		battlement.Vertices.size()+
		tetrahedron.Vertices.size()+
		parrellegram.Vertices.size();
	...
	for (size_t i = 0; i < battlement.Vertices.size(); ++i, ++k)
	{
		vertices[k].Pos = battlement.Vertices[i].Position;
		vertices[k].Normal = battlement.Vertices[i].Normal;
	}
	...
	for (size_t i = 0; i < parrellegram.Vertices.size(); ++i, ++k)
	{
		vertices[k].Pos = parrellegram.Vertices[i].Position;
		vertices[k].Normal = parrellegram.Vertices[i].Normal;
	}
	...
	indices.insert(indices.end(), std::begin(battlement.GetIndices16()), std::end(battlement.GetIndices16()));
	indices.insert(indices.end(), std::begin(tetrahedron.GetIndices16()), std::end(tetrahedron.GetIndices16()));
	indices.insert(indices.end(), std::begin(parrellegram.GetIndices16()), std::end(parrellegram.GetIndices16()));
	...
	geo->DrawArgs["battlement"] = battlementSubmesh;
	geo->DrawArgs["tetrahedron"] = tetraSubmesh;
	geo->DrawArgs["parrellegram"] = parrellegramSubmesh;
...
void LitColumnsApp::BuildMaterials()
{
	...
	mMaterials["base"] = std::move(Base);
	mMaterials["tile0"] = std::move(tile0);
	mMaterials["skullMat"] = std::move(skullMat);
	mMaterials["blueDiamond"] = std::move(blueDiamond);
...
void LitColumnsApp::BuildRenderItems()
{
	...
	///////////DIAMOND IS HEREREERERERERERERERERE
	auto diamondRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&diamondRitem->World, XMMatrixScaling(.1f, .1f, .1f)*XMMatrixRotationX(XMConvertToRadians(195))*XMMatrixTranslation(0.6f, 3.0f, -.4f));
	diamondRitem->ObjCBIndex = Num_Object++;
	diamondRitem->Mat = mMaterials["blueDiamond"].get();
	...
	XMStoreFloat4x4(&diamondRitem2->World, XMMatrixScaling(.1f, .1f, .1f)*XMMatrixRotationX(XMConvertToRadians(195))*XMMatrixTranslation(-0.6f, 3.0f, -.4f));
	diamondRitem2->ObjCBIndex = Num_Object++;
	diamondRitem2->Mat = mMaterials["redDiamond"].get();
	...
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(10.0f, 10.0f, 10.0f)*XMMatrixTranslation(0.0f, 2.5f, 0.0f));
	XMStoreFloat4x4(&boxRitem->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));
	boxRitem->ObjCBIndex = Num_Object++;
	boxRitem->Mat = mMaterials["base"].get();
	...
	XMStoreFloat4x4(&gridRitem->TexTransform, XMMatrixScaling(28.0f, 28.0f, 1.0f));
	gridRitem->ObjCBIndex = Num_Object++;
	...
	skullRitem->ObjCBIndex = Num_Object++;
	...
	//For loop for Pyramid roof stuff
	XMMATRIX pyramidTransform = XMMatrixScaling(2.0f, 2.0f, 2.0f);
	for (int i = 0; i < 3; ++i)
	{
		...
		leftPyrRitem->StartIndexLocation = leftPyrRitem->Geo->DrawArgs["pyramid"].StartIndexLocation;
		leftPyrRitem->BaseVertexLocation = leftPyrRitem->Geo->DrawArgs["pyramid"].BaseVertexLocation;
		...
		rightPyrRitem->StartIndexLocation = rightPyrRitem->Geo->DrawArgs["pyramid"].StartIndexLocation;
		rightPyrRitem->BaseVertexLocation = rightPyrRitem->Geo->DrawArgs["pyramid"].BaseVertexLocation;

		mAllRitems.push_back(std::move(leftPyrRitem));
		mAllRitems.push_back(std::move(rightPyrRitem));
	}

	//For loop for battlement roof stuff
	XMMATRIX battlementTransform = XMMatrixScaling(1.0f, 1.0f, 1.0f);
	for (int i = 0; i < 2; ++i)
	{
		...
		BatRitem->Mat = mMaterials["battlement"].get();
		BatRitem->Geo = mGeometries["shapeGeo"].get();
		BatRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
		BatRitem->IndexCount = BatRitem->Geo->DrawArgs["battlement"].IndexCount;
		BatRitem->StartIndexLocation = BatRitem->Geo->DrawArgs["battlement"].StartIndexLocation;
		BatRitem->BaseVertexLocation = BatRitem->Geo->DrawArgs["battlement"].BaseVertexLocation;

		mAllRitems.push_back(std::move(BatRitem));
	}

	auto wedgeRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&wedgeRitem->World, XMMatrixScaling(5.0f, 0.5f, 5.0f)*XMMatrixRotationY(XMConvertToRadians(-90))*XMMatrixTranslation(-3.0f, 0.0f, -12.5f));
	wedgeRitem->ObjCBIndex = Num_Object++;
	wedgeRitem->Mat = mMaterials["wedge"].get();
	...
	XMStoreFloat4x4(&tetraRitem->World, XMMatrixScaling(1.0f, 1.0f, 1.0f)*XMMatrixRotationX(XMConvertToRadians(0))*XMMatrixTranslation(3.6f, 4.0f, -2.4f));
	tetraRitem->ObjCBIndex = Num_Object++;
	tetraRitem->Mat = mMaterials["blueDiamond"].get();
	...
	for(int i = 0; i < 5; ++i)
	{
		...
		XMMATRIX leftCylWorld = XMMatrixTranslation(-9.0f, 1.5f, -10.0f + i*5.0f);
		XMMATRIX rightCylWorld = XMMatrixTranslation(+9.0f, 1.5f, -10.0f + i*5.0f);

		XMMATRIX leftSphereWorld = XMMatrixTranslation(-9.0f, 3.5f, -10.0f + i*5.0f);
		XMMATRIX rightSphereWorld = XMMatrixTranslation(+9.0f, 3.5f, -10.0f + i*5.0f);

		XMStoreFloat4x4(&leftCylRitem->World, rightCylWorld);
		XMStoreFloat4x4(&leftCylRitem->TexTransform, brickTexTransform);
		leftCylRitem->ObjCBIndex = Num_Object++;
		...
		rightCylRitem->ObjCBIndex = Num_Object++;
		...
		leftSphereRitem->ObjCBIndex = Num_Object++;
		...
		rightSphereRitem->ObjCBIndex = Num_Object++;
